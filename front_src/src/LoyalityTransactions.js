...
class LoyalityTransactions extends React.Component {
  ...
  fetchTags = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/loyality_transactions/?token=${this.props.query.token}&tags=${name}`
      )
  ...
  handleDelete = (id) => {
    ...
    axios
      .delete(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/loyality_transactions/${row.id}/`,
  ...
  edit_request = (id, payment, row) => {
    ...
    if (Object.keys(edit_dict).length !== 0) {
      axios.patch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/loyality_transactions/${id}/?token=${this.props.query.token}`,
        edit_dict
      );
      const newData = [...this.state.dataSource];
          ...
          >
            <Panel header="Фильтры поиска платежей..." key="1">
              ...
              >
                ...
                <Form.Item name="loyality_card_number">
                  {/* <InputNumber
										// mode="multiple"
