...
class Integrations extends Component {
  ...
  componentDidMount() {
    axios
      .get(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/check_pair/?token=${this.props.query.token}`
      )
      .then((data) => {
        if (data.data.result !== "paired") {
          ...
          this.setState({
            actions: [
              <IntegrationModal
                query={this.props.query}
                button_name={"Обновить токен"}
              />,
            ],
            status: <p style={{ color: "red" }}>Необходимо обновить токен</p>,
          });
        } else {
  ...
  ws_connect = () => {
    this.websocket = new WebSocket(
      `wss://${process.env.REACT_APP_APP_URL}/ws/${this.props.query.token}/`
    );
  ...
  disconnect = () => {
    axios
      .get(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/integration_unpair/?token=${this.props.query.token}`
      )
  ...
  connect = () => {
    axios
      .get(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/integration_on/?token=${this.props.query.token}`
      )
  ...
  render() {
    return (
      ...
      >
        <Meta title="AmoCRM" description={this.state.status} />
      </Card>
    );
  }
}
