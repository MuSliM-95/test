...
class EditPayment extends React.Component {
  ...
  showModal = () => {
    ...
    const caPhones = caMeta
      .map((item) => {
        return { label: item.phone, value: item.id };
      })
      .filter((item) => item.label);
    const caInns = caMeta
      .map((item) => {
        return { label: item.inn, value: item.id };
      })
      .filter((item) => item.label);
    ...
    if (payment.contragent) {
      axios
        .get(
          `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/${payment.contragent}/`,
    ...
    if (payment.article_id) {
      axios
        .get(
          `https://${process.env.REACT_APP_APP_URL}/api/v1/articles/${payment.article_id}/`,
          { params: { token: this.props.token } }
        )
        .then((response) => {
          this.setState({
  ...
  onFinish = async (values) => {
    ...
    if (
      ...
      await axios.put(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/${current_contragent.id}/?token=${this.props.token}`,
        body
      );
    }
    ...
    if (
      ...
      await axios.put(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/articles/${current_article.id}/?token=${this.props.token}`,
        artBody
      );
    }
    ...
    if (!isArticleCleared && isNewArticle) {
      const resp = await axios.post(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/articles/?token=${this.props.token}`,
        artBody
      );
    ...
    if (
      requestBody.amount ||
      requestBody.tax ||
      requestBody.tax_type ||
      requestBody.amount_without_tax
    ) {
    ...
    if (
      parseFloat(requestBody.amount) === parseFloat(this.props.payment.amount)
    ) {
      delete requestBody.amount;
    }

    if (
      requestBody.project_id === "0" &&
      this.props.payment.project_id === null
    ) {
    ...
    if (this.props.payment.raspilen && requestBody.project_id) {
      confirm({
        ...
        content:
          "Вы уверены? При смене проекта родительского платежа слетят проекты на всех дочерних",

        onOk: async () => {
          await axios.put(
            `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/${this.props.payment.id}/?token=${this.props.token}`,
            requestBody
          );
        },
  ...
  fetchNames = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments_meta/?token=${this.props.token}&name=${name}`
      )
  ...
  fetchTags = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments_meta/?token=${this.props.token}&tags=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
  ...
  fetchProjects = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/projects/?token=${this.props.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/projects/?token=${this.props.token}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
  ...
  fetchPayboxes = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payboxes/?token=${this.props.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
            ...
            return body.result.map((payment) => {
              ...
              return {
                label: (
                  <>
                    {payment.name}:{" "}
                    <font color={color}>{payment.balance} руб.</font>
                  </>
                ),
                value: payment.id,
              };
      ...
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payboxes/?token=${this.props.token}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
  ...
  findArticle = async (id) => {
    return fetch(
      `https://${process.env.REACT_APP_APP_URL}/api/v1/articles/${id}/?token=${this.props.token}`
    )
      .then((response) => response.json())
      .then((body) => {
        return body;
    ...
    return (
      <>
        <Button
          ...
          icon={<EditOutlined />}
          onClick={() => this.showModal(this.props.payment)}
        />
        <Modal
          open={modalOpened}
          width={550}
        ...
        >
          {isContrLoaded && isArticleLoaded ? (
            ...
            >
              <Form.Item name="name" label="Название платежа">
                <AutoComplete
                  style={{ width: 300 }}
                  options={namesMeta}
                  allowClear={true}
                  ...
                  {!isNewArticle ? (
                      ...
                      {current_article ? (
                        <div>
                          <Alert
                            ...
                            message={
                              <div>
                                Выбран <b>{current_article.name}</b>
                              </div>
                            }
                            showIcon
                            action={
                    ...
                    <div>
                      <Alert
                        ...
                        message={
                          <div>
                            Вы создаете <b>{newArticleName}</b>
                          </div>
                        }
                        showIcon
                        action={
              ...
              <Form.Item name="article_id" label="Статья платежа">
                <ArticleAutocomplete
                  api={this.api}
              ...
              <Form.Item name="tags" label="Теги платежа">
                <Select
                  style={{ width: 300 }}
                  options={tagsMeta}
                  allowClear={true}
              ...
              <Form.Item name="project_id" label="Проект платежа">
                <DebounceSelect
              ...
              {this.state.paymentType === "transfer" ? (
                <>
                  <Button
                    onClick={this.swap_payboxes}
                    style={{ marginLeft: 100, marginBottom: 20 }}
                    icon={<SwapOutlined />}
                  >
                    Поменять счета местами
                  </Button>
                  <Form.Item name="paybox_to" label={"Счет зачисления"}>
                    <DebounceSelect
                      placeholder="Выберите счет"
                      fetchOptions={this.fetchPayboxes}
                      removeIcon={null}
              ...
              >
                <Switch
                  onChange={(checked) =>
                    this.setState({ paymentStatus: checked })
                  }
                />
              </Form.Item>
              ...
              <Form.Item label={"Сумма платежа"} name="amount_without_tax">
                <InputNumber
                  ...
                  onChange={(amount) =>
                    this.setState({ amount: amount, amountIsChanged: true })
                  }
                  precision={2}
                  min="0.01"
                  step="0.01"
              ...
              {this.state.p_type === "transfer" ? null : (
                <>
                  ...
                  <TextBlock
                    tax={
                      this.state.tax ? this.state.tax : this.props.payment.tax
                    }
                    tax_type={
                      this.state.tax_type
                        ? this.state.tax_type
                        : this.props.payment.tax_type
                    }
                    amount={
                      this.state.amount
                        ? this.state.amount
                        : this.props.payment.amount_without_tax
                    }
                  />
                </>
              )}
              ...
              {this.props.payment.type !== "transfer" ? (
                <>
                    ...
                    rules={[
                      {
                        pattern: "^\\d+$",
                        message:
                          "Телефон не должен содержать символы кроме цифр",
                      },
                    ]}
                  >
                    <NumericAutoComplete
                  ...
                  <Form.Item label="Примечание" name="contragent_desc">
                    <TextArea
                      style={{ width: 320 }}
