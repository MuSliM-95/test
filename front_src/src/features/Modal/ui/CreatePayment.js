// Модальное окно создания платежа на странице "Платежи"
import {
  AutoComplete,
  Button,
  DatePicker,
...
class CreatePayment extends React.Component {
  ...
  handleOk = async (values) => {
    ...
    await axios.post(
      `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/?token=${this.props.token}`,
      requestBody
    );
      ...
      <>
        ...
        >
          ...
          >
            ...
            <Form.Item name="paybox" label={this.state.pbox_label}>
              <DebounceSelect
            ...
            <Form.Item
              ...
              onChange={(event) => {
                ...
                if (event.target.value === "transfer") {
                  this.setState({
                    pbox_label: "Счет списания",
                    pbox_to_label: "Счет зачисления",
                  });
                } else {
            ...
            {this.state.paymentType === "transfer" ? (
              <Form.Item name="paybox_to" label={this.state.pbox_to_label}>
                <DebounceSelect
                  placeholder="Выберите счет"
                  fetchOptions={this.fetchPayboxes}
                  removeIcon={null}
              ...
              <>
                ...
                <Form.Item label="Тип налога" name="tax_type">
                  <Radio.Group
                    onChange={(event) =>
                      this.setState({ tax_type: event.target.value })
                    }
                  >
                ...
                <TextBlock
                  ...
                  tax_type={
                    this.state.tax_type
                      ? this.state.tax_type
                      : init_values.tax_type
                  }
                  amount={
                    this.state.amount ? this.state.amount : init_values.amount
                  }
                />
              </>
            )}
