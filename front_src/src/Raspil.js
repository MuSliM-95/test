...
class Raspil extends React.Component {
    ...
    columns: [
      ...
      },
      {
        ...
        render: (value, row) => {
          return (
            <Select
              onChange={(val) => this.handleChangeData(row, "project", val)}
              defaultValue={value}
              style={{ width: "100%" }}
            >
      ...
      },
      {
        ...
        render: (value, row) => {
          return (
            <Select
              onChange={(val) => this.handleChangeData(row, "ca", val)}
              defaultValue={value}
              style={{ width: "100%" }}
            >
              {this.props.caData}
  ...
  handleOk = (item) => {
    ...
    if (item.raspil_type === "nequals") {
      ...
      last_pay = parseFloat(
        (this.props.payment.amount % item.raspil_amount).toFixed(2)
      );
      for (let i = 0; i < pay_count; i++) {
        ds.push({
          amount: item.raspil_amount,
          project: "0",
          ca: "0",
          key: i + 1,
        });
            ...
            >
              ...
              >
                <Radio.Group onChange={this.handleName}>
                  <Radio.Button value="nequals">
                    Разбить на части по ...
                  </Radio.Button>
                  <Radio.Button value="equals">
                    Разбить на ... равных частей
                  </Radio.Button>
                </Radio.Group>
              </Form.Item>

              <Form.Item name="raspil_amount" label={this.state.type_name}>
