...
class CATable extends React.Component {
  ...
  handleDelete = (id) => {
    ...
    axios
      .delete(`https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/${row.id}/`, {
  ...
  edit_request = (row) => {
    ...
    axios
      .put(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/${row.id}/?token=${this.props.query.token}`,
        body
      )
      .then((response) => {
          ...
          pagination={{
            ...
            onChange: (page) => {
              this.setState(
                ...
                this.fetch(page, `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/`)
              );
            },
            pageSize: 35,
