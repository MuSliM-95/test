...
class EditableTable extends React.Component {
        ...
        render: (date, row) => {
          return (
            <DatePicker
              ...
              onChange={(date) =>
                this.handleChangeDate(dayjs(date).utc().unix(), row)
              }
      ...
      },
      {
        ...
        render: (paymentType, row) => {
          if (paymentType === "incoming") {
            ...
            content = (
              <>
                Зачисление на счет <b>{pbox_from_name}</b> <br />
                На сумму: <b>{row.amount} руб.</b>
              </>
            );
            return (
              <Popover title={"Счет зачисления"} content={content}>
          ...
          if (paymentType === "outgoing") {
            ...
            content = (
              <>
                Списание с счета <b>{pbox_from_name}</b> <br />
                На сумму: <b>{row.amount} руб.</b>
              </>
            );
            return (
              <Popover title={"Счет списания"} content={content}>
            ...
            if (pbox_from_name && pbox_to_name) {
              content = (
                <>
                  Перевод со счета <b>{pbox_from_name}</b> на счет{" "}
                  <b>{pbox_to_name}</b>
                  <br />
                  На сумму: <b>{row.amount} руб.</b>
                </>
            ...
            return (
              <Popover title={"Перевод со счета на счет"} content={content}>
        ...
        render: () => {
          return (
            <Popover
              content={
                <>
                  Повторы в разработке <FrownOutlined />
                </>
              }
            >
              <Button
          ...
          if (
            ...
            return (
              ...
              >
                <Switch
                  checked={checked}
                  onClick={(checked) => this.handleChangeDebCred(checked, row)}
                />
              </Popover>
            );
          }
  ...
  fetchTags = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments_meta/?token=${this.props.query.token}&tags=${name}`
      )
  ...
  handleDelete = (id) => {
    ...
    axios
      .delete(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/${row.id}/`,
        {
  ...
  fetchNames = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/?token=${this.props.query.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/?token=${this.props.query.token}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
  ...
  fetchProjects = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/projects/?token=${this.props.query.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/projects/?token=${this.props.query.token}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
  ...
  fetchPaybox = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payboxes/?token=${this.props.query.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payboxes/?token=${this.props.query.token}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
  ...
  fetchContr = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/?token=${this.props.query.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/?token=${this.props.query.token}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
  ...
  handleSave = (row) => {
    ...
    if (
      finded_row.tax === 0 &&
      parseFloat(item["amount"]) !== parseFloat(finded_row.amount)
    ) {
  ...
  edit_request = (id, payment, row) => {
    ...
    if (Object.keys(edit_dict).length !== 0) {
      axios.put(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/${id}/?token=${this.props.query.token}`,
        edit_dict
      );
    } else {
  ...
  fetch = (page, pars, sort = null) => {
    ...
    axios
      .get(`https://${process.env.REACT_APP_APP_URL}/api/v1/payments/`, params)
      .then((response) => {
        this.setState({
    ...
    var newurl =
      ...
      "?" +
      serialize({ ...this.props.query, ...filters });
            ...
            <Panel header="Фильтры поиска платежей..." key="1">
              ...
              >
                <Form.Item
                  name="relship"
                  initialValue={
                    this.props.query.relship ? this.props.query.relship : "all"
                  }
                >
                ...
                <Form.Item name="name" initialValue={this.props.query.name}>
                  {/* <AutoComplete
                ...
                <Form.Item name="dates">
                  <RangePicker
                    ...
                    initialValue={[
                      this.props.query.datefrom
                        ? dayjs(this.props.query.datefrom)
                        : false,
                      this.props.query.dateto
                        ? dayjs(this.props.query.dateto)
                        : false,
                    ]}
                    format={"DD-MM-YYYY"}
                  />
                ...
                <Form.Item name="paybox" initialValue={this.props.query.paybox}>
                  {/* <AutoComplete
										style={{ width: 200, marginTop: 10 }}
