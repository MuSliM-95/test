...
import { ContragentAutocomplete } from 'src/components/shared'
import { ContractAutocomplete } from "src/components/shared";
import { NomenclatureAutoComplete } from "src/components/shared";
import { WareHousesAutocomplete } from "src/components/shared";
import { OrganizationAutoComplete } from "src/components/shared"
...
class NewDocsSales extends React.Component {
  constructor(props) {
    super(props);
  ...
  finish = (action) => {
    ...
    if (!this.state.current_warehouse || !this.state.current_organization) {
      ...
      if (action === "add_proc") {
        body.status = true;
        body.paid_rubles = this.finRef.current.getFieldValue("rubles") || 0;
        body.paid_lt = this.finRef.current.getFieldValue("lt") || 0;
        body.sum = body.paid_rubles + body.paid_lt;
      ...
      axios
        .post(
          `https://${process.env.REACT_APP_APP_URL}/api/v1/docs_sales/?token=${this.props.token}`,
          [body]
        )
        .then((response) => {
  ...
  render() {
    ...
    return (
      <>
        <Button onClick={showModal}>Добавить документ продажи</Button>
        <Modal
          width={1800}
          destroyOnClose={true}
          footer={null}
          ...
          <Layout>
            ...
            <Sider style={siderStyle}>
              ...
              >
                ...
                <Form.Item label="Баллами:" name={"lt"}>
                  <InputNumber
                    ref={this.inputLoyality}
                    style={{ width: "100%" }}
                    precision={2}
                    min={0}
                ...
                <Form.Item label="Рублями:" name={"rubles"}>
                  <InputNumber
                    style={{ width: "100%" }}
                    precision={2}
                    min={0}
