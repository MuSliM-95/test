// Card на странице "Интеграции"
import React, { Component } from 'react';
import { CheckOutlined, StopOutlined } from '@ant-design/icons';
import { Card, Modal } from 'antd';
import { CreateIntegration } from 'src/components/features/Modal';
        ...
        axios.get(`https://${process.env.REACT_APP_APP_URL}/api/v1/check_pair/?token=${this.props.query.token}`)
            .then((data) => {
                ...
                if (data.data.result !== "paired") {
                    this.setState({ actions: [<CreateIntegration query={this.props.query} button_name={"Установить виджет"} />] })
                }

                else if (data.data.integration_status === "need_to_refresh") {
                    this.setState({ actions: [<CreateIntegration query={this.props.query} button_name={"Обновить токен"} />], status: <p style={{ color: "red" }}>Необходимо обновить токен</p> })
                }
