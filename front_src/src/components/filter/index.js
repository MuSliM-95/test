...
import { Button, Form, Select, Collapse, DatePicker } from "antd";
...
const Filter = ({ query, onChange }) => {
  ...
  const fetchNames = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/?token=${query.token}&name=${name}`
      )
      ...
      if (openFilter) {
        return fetch(
          `https://${process.env.REACT_APP_APP_URL}/api/v1/payments/?token=${query.token}`
        )
          .then((response) => response.json())
          .then((body) => {
            return body;
  ...
  const fetchProjects = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/projects/?token=${query.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      if (openFilter) {
        return fetch(
          `https://${process.env.REACT_APP_APP_URL}/api/v1/projects/?token=${query.token}`
        )
          .then((response) => response.json())
          .then((body) => {
            return body;
  ...
  const fetchPaybox = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/payboxes/?token=${query.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      if (openFilter) {
        return fetch(
          `https://${process.env.REACT_APP_APP_URL}/api/v1/payboxes/?token=${query.token}`
        )
          .then((response) => response.json())
          .then((body) => {
            return body;
  ...
  const fetchContr = async (name) => {
    if (name) {
      return fetch(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/?token=${query.token}&name=${name}`
      )
        .then((response) => response.json())
        .then((body) => {
          return body;
      ...
      if (openFilter) {
        return fetch(
          `https://${process.env.REACT_APP_APP_URL}/api/v1/contragents/?token=${query.token}`
        )
          .then((response) => response.json())
          .then((body) => {
            return body;
    ...
    >
      <Panel header="Фильтры поиска платежей..." key="1">
        <Form
          ...
          onFinish={onFinish}
        >
          <Form.Item name="relship" initialValue={state.relship}>
          ...
          <Form.Item name="payment_type" initialValue={state.payment_type}>
          ...
          <Form.Item name="name" initialValue={state.name}>
            <DebounceSelectFil
          ...
          <Form.Item name="project" initialValue={state.project}>
            <DebounceSelectFil
          ...
          <Form.Item name="paybox" initialValue={state.paybox}>
            <DebounceSelectFil
          ...
          <Form.Item name="contragent" initialValue={state.contragent}>
            <DebounceSelectFil
