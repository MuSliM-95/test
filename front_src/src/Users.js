...
class UsersTable extends React.Component {
    ...
    this.columns = [
        ...
        render: (photo_src) => {
          return (
            <Avatar
              src={
                <Image
                  src={`https://${process.env.REACT_APP_APP_URL}/${photo_src}`}
                />
              }
              size={60}
            />
      ...
      },
      {
        ...
        render: (username) => {
          if (username) {
            ...
            return (
              <Alert
                message="У этого пользователя нет юзернейма"
                type="warning"
              />
            );
          }
      ...
      },
      {
        ...
        render: (checked, row) => {
          ...
          return (
            <Tooltip title="Нельзя изменить статус админа!">
              <WarningOutlined />
            </Tooltip>
          );
        },
  ...
  handleChangeStatus = (checked, row) => {
    axios
      .put(
        `https://${process.env.REACT_APP_APP_URL}/api/v1/cashbox_users/?token=${this.props.query.token}&user_id=${row.id}&status=${checked}`
      )
      .then((response) => {
  ...
  fetch = (page, url = {}) => {
    ...
    axios
      .get(url, {
        params: { token: this.props.query.token, limit: limit, offset: offset },
      })
      .then((response) => {
        this.setState({
        ...
        {this.props.invite ? (
          <div style={{ marginBottom: 10 }}>
            ...
            <a
              ...
              target={"_blank"}
              className="button"
              href={`https://t.me/tablecrmbot?start=${this.props.invite}`}
            >{`https://t.me/tablecrmbot?start=${this.props.invite}`}</a>
          </div>
        ) : null}
        <Table
