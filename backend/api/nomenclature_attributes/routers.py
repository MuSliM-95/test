...
from database.db import categories, database, manufacturers, nomenclature, nomenclature_barcodes, prices, price_types, \
    warehouse_register_movement, warehouses, units, warehouse_balances, nomenclature_attributes, nomenclature_attributes_value, nomenclature_groups
...
@router.post("/nomenclature/attributes-value", response_model=schemas.AttributeValueResponse)
async def new_nomenclature_attribute_value(
    ...
    query = select(nomenclature.c.id).where(nomenclature.c.id == attribute_value_data.nomenclature_id)
    ...
    # Проверка существования атрибутов и сбор значений для вставки
    attributes_to_insert = []
    for attribute in attribute_value_data.attributes:
        # Проверка существования атрибута
        query = select(nomenclature_attributes.c.id).where(nomenclature_attributes.c.id == attribute.attribute_id)
        attribute_record = await database.fetch_one(query)
        if not attribute_record:
            raise HTTPException(
                status_code=404, detail=f"Атрибут с ID '{attribute.attribute_id}' не найден"
            )

        # Обработка значения: список или строка
    ...
    # Вставка значений атрибутов в базу данных
    try:
    ...
    return schemas.AttributeValueResponse(nomenclature_id=attribute_value_data.nomenclature_id, attributes=attributes_to_insert)
...
async def get_nomenclature_attributes(nomenclature_id: int, token: str):
    ...
    query = (
        ...
        .order_by(nomenclature_attributes.c.name)  # Упорядочиваем по имени атрибута
    )
...
async def create_nomenclature_group(token: str, data: schemas.NomenclatureRelations):
    ...
    # Генерация новой group_id
    group_id_result = await database.fetch_one(
        select(func.max(nomenclature_groups.c.group_id).label("max_group_id"))
    )
    new_group_id = (group_id_result["max_group_id"] or 0) + 1  # Следующее значение group_id

    # Вставка записей для каждой номенклатуры в группу
    try:
        async with database.transaction():
            for nomenclature_id in data.nomenclature_ids:
                query = insert(nomenclature_groups).values(
...
async def get_group_for_nomenclature(token: str, nomenclature_id: int):
    ...
    query = select(nomenclature_groups.c.group_id).where(
        nomenclature_groups.c.nomenclature_id == nomenclature_id
    )
...
async def add_nomenclature_to_group(token: str, group_id: int, nomenclature_id: int):
    ...
    group_exists = await database.fetch_one(
        select(nomenclature_groups.c.group_id).where(nomenclature_groups.c.group_id == group_id)
    )
    if not group_exists:
    ...
    already_in_group = await database.fetch_one(
        select(nomenclature_groups.c.id).where(
            (nomenclature_groups.c.group_id == group_id) &
            (nomenclature_groups.c.nomenclature_id == nomenclature_id)
        )
    )
    if already_in_group:
...
async def get_nomenclatures_by_group(token: str, group_id: int):
    ...
    # Запрос на получение только ID номенклатур
    query = (
        select(
        ...
        .select_from(
            ...
            .join(
                nomenclature_groups,
                nomenclature.c.id == nomenclature_groups.c.nomenclature_id
            )
        )
        .where(nomenclature_groups.c.group_id == group_id)
    )
