...
class GetDocsSalesListByDeliveryDateQuery:
    ...
    @staticmethod
    async def _get_items_and_count_in_db(cashbox_id, start_date, end_date, filters) -> Tuple[List[Dict[str, Any]], int]:
        query = DocsSalesListRepository().query_by_delivery_date(cashbox_id, start_date, end_date)
        count_query = DocsSalesListRepository().get_count_list_by_delivery_date_query(cashbox_id, start_date, end_date)
