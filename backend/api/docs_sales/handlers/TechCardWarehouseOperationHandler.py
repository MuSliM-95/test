...
class TechCardWarehouseOperationHandler(
    IEventHandler[TechCardWarehouseOperationMessage]
):
    async def __call__(
        ...
        async with database.transaction():
            ...
            if tech_card_type == "automatic":
                query_tech_card_operation_components = select(
                    TechOperationComponentDB
                ).where(
                    TechOperationComponentDB.operation_id
                    == event["tech_card_operation_uuid"]
                )
                tech_card_operation_db = await database.fetch_all(
                    query_tech_card_operation_components
                )
                to_insert = []
                for component in tech_card_operation_db:
                    query_warehouse_balance = (
                        warehouse_balances.select()
                        .where(
                            warehouse_balances.c.cashbox_id == user_cashbox_id,
                            warehouse_balances.c.nomenclature_id
                            == component.nomeclature_id,
                        )
                        .order_by(warehouse_balances.c.id.desc())
                    )
                    warehouse_balances_db = await database.fetch_one(
                        query_warehouse_balance
                    )
                    current_amount = (
                        warehouse_balances_db.current_amount
                ...
                current_amount = (
                    warehouse_balances_db.current_amount if warehouse_balances_db else 0
                )
