...
class GoodsRepository:
    ...
    #
    # @staticmethod
    # async def fetch_goods_by_docs_ids(doc_ids: list[int], show_goods=True) -> dict[int, list[dict]]:
    #     """Получает товары для указанных ID документов и группирует их по doc_id,
    #     сразу добавляя информацию о номенклатуре и единицах измерения"""
    #     if not show_goods:
    #         return {}
    #     if not doc_ids:
    #         return {}
    ...
    #     list_col = [c for c in docs_sales_goods.c if c.name not in ("created_at", "updated_at")]
    #
    #     # JOIN таблиц товаров, номенклатуры и единиц
    #     goods_join = (
    #         docs_sales_goods
    #         .join(nomenclature, docs_sales_goods.c.nomenclature == nomenclature.c.id, isouter=True)
    #         .join(units, docs_sales_goods.c.unit == units.c.id, isouter=True)
