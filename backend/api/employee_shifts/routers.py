...
async def start_shift(
    ...
    try:
        ...
        shift_id = await database.execute(employee_shifts.insert().values(shift_data))
        
        # Получаем созданную смену
        created_shift = await database.fetch_one(
            employee_shifts.select().where(employee_shifts.c.id == shift_id)
        )
        ...
        raise HTTPException(
            ...
            detail=f"Ошибка при начале смены: {str(e)}"
        )
...
async def end_shift(
    ...
    try:
        ...
        updated_shift = await database.fetch_one(
            employee_shifts.select().where(employee_shifts.c.id == active_shift.id)
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка при завершении смены: {str(e)}"
        )
...
async def create_break(
    ...
    try:
        ...
        # Проверяем валидность продолжительности перерыва
        if request.duration_minutes <= 0 or request.duration_minutes > 480:  # Максимум 8 часов
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка при создании перерыва: {str(e)}"
        )
...
async def end_break(
    ...
    try:
        ...
        # Получаем обновленную смену
        updated_shift = await database.fetch_one(
            employee_shifts.select().where(employee_shifts.c.id == break_shift.id)
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка при завершении перерыва: {str(e)}"
        )
...
async def get_shift_status(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Ошибка при получении статуса смены: {str(e)}"
        )
