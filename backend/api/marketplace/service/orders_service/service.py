...
from database.db import database, nomenclature, warehouse_balances, warehouses
    ...
    async def create_order(
        ...
        for good in order_request.goods:
            cashbox_query = select(nomenclature.c.cashbox).where(
                nomenclature.c.id == good.nomenclature_id
            )
            ...
            if good.warehouse_id is None:
                if all([order_request.client_lat, order_request.client_lon]):
                    ...
                    raise HTTPException(
                        status_code=422,
                        detail="Нужно указать либо склад, либо координаты клиента",
                    )
            else:
        ...
        contragent_id = await self._get_contragent_id_by_phone(
            order_request.contragent_phone
        )
