...
async def get_chats(
    ...
    search: Optional[str] = Query(None, description="Поиск по имени контакта или названию объявления"),
            ...
            if channel and channel['type'] == 'AVITO' and chat.get('external_chat_id'):
                ...
                if client:
                    try:
                        ...
                        if message.image_url and (message.message_type == "IMAGE" or (has_image and has_text)):
                            try:
                                import aiohttp
                                import base64
                                import io
                                        ...
                                        async with session.get(image_url, headers=headers) as img_response:
                                            if img_response.status == 200:
                                                ...
                                                if not content_type.startswith('image/'):
                                                    ...
                                                    if len(image_data) == 0:
                                                        image_data = None
                                                    elif len(image_data) > 24 * 1024 * 1024:
                                                        image_data = None
                                                    else:
                                ...
                                if image_data and len(image_data) > 0 and len(image_data) <= 24 * 1024 * 1024:
                                    try:
                                        ...
                                        async with s3_session.client(**s3_data) as s3:
                                            await s3.upload_fileobj(io.BytesIO(image_data), bucket_name, file_link)
                                        
                                        image_url_for_db = file_link
                                        logger.info(f"File saved to S3: {file_link}")
                                    except Exception as e:
                                        logger.error(f"Failed to save file to S3: {e}", exc_info=True)
                                
                                if message.message_type == "IMAGE" and image_data and len(image_data) > 0 and len(image_data) <= 24 * 1024 * 1024:
                                    try:
                                        upload_result = await client.upload_image(image_data, filename)
                                        
                                        if upload_result:
                                            if isinstance(upload_result, tuple):
                                                image_id, avito_image_url = upload_result
                                                if avito_image_url:
                        ...
                        if (message.message_type == "IMAGE" or (has_image and has_text)) and image_url_for_db:
                            try:
                                ...
                                logger.info(f"File saved to pictures table for message {db_message['id']}: {image_url_for_db}")
                            except Exception as e:
                                logger.warning(f"Failed to save file for message {db_message['id']}: {e}", exc_info=True)
                        ...
                        if has_image and has_text and db_message_text:
                            if image_id:
                                try:
                                    ...
                                    logger.error(f"Failed to send IMAGE message to Avito: {e}", exc_info=True)
                            ...
                            try:
                                ...
                                logger.error(f"Failed to send TEXT message to Avito: {e}", exc_info=True)
                                await crud.update_message(db_message_text['id'], status="FAILED")
                        else:
    ...
    if messages:
        ...
        if message_ids:
            try:
                ...
                logger.warning(f"Failed to load images for messages: {e}", exc_info=True)
