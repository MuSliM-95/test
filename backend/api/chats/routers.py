...
async def get_chats(
    ...
    search: Optional[str] = Query(
        None, description="Поиск по имени, телефону или external_chat_id"
    ),
                ...
                if client:
                    try:
                                        ...
                                        if img_response.status == 200:
                                            ...
                                            if not content_type.startswith("image/"):
                                                ...
                                                if len(image_data) == 0:
                                                    ...
                                                    len(image_data) > 24 * 1024 * 1024
                                                ):  # 24 МБ
                                                    image_id = None
                                                else:
                                                    filename = (
                                                        image_url.split("/")[-1].split(
                                                            "?"
                                                        )[0]
                                                        or "image.jpg"
                                                    )
                        ...
                        if message.message_type == "IMAGE":
                            if not image_id:
                                logger.warning(
                                    f"Cannot send IMAGE message: image upload failed. image_url: {message.image_url}"
                                )
                                raise Exception(
                                    "Cannot send IMAGE message: image upload failed"
                                )
                            ...
                            raise Exception(
                                "Cannot send message: image upload failed and no text provided"
                            )
                        ...
                        if external_message_id:
                            ...
                            if (
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"Failed to save image file for message {db_message['id']}: {e}"
                                    )
                    except Exception as e:
                        logger.error(
