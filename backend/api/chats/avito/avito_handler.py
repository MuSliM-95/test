...
class AvitoHandler:
    ...
    async def handle_message_event(
        webhook: AvitoWebhook, cashbox_id: int, channel_id: Optional[int] = None
    ) -> Dict[str, Any]:
        try:
                    ...
                    if not avito_channel:
                        ...
                        if client:
                            ...
                            if not user_phone:
                                ...
                                logger.warning(
                                    f"No users found in chat info for {chat_id_external}"
                                )
            ...
            if not user_name:
                if sender_type == "CLIENT":
                    user_name = (
                        f"Avito User {author_id}" if author_id else "Unknown User"
                    )
                else:
                    user_name = None
                ...
                try:
                    ...
                    if message_type == "voice":
                        if isinstance(message_content, dict):
                            file_url = message_content.get(
                                "url"
                            ) or message_content.get("voice_url")
                            if not file_url:
                                voice_id = message_content.get("voice_id")
                                if voice_id:
                                    try:
                                        ...
                                        if chat_data:
                                            ...
                                            avito_channel = await crud.get_channel(
                                                channel_id
                                            )
                                            if (
                                                avito_channel
                                                ...
                                                if voice_client:
                                                    file_url = await voice_client.get_voice_file_url(
                                                        voice_id
                                                    )
                                    except Exception as e:
                                        logger.warning(
            ...
            if channel_type == "AVITO":
                ...
                if not channel:
                    channel = await crud.get_channel_by_cashbox(
                        cashbox_id, channel_type
                    )
                    if channel:
                        logger.warning(
                            f"Using fallback channel {channel['id']} for cashbox {cashbox_id} (user_id not matched)"
                        )

                if not channel:
                    logger.warning(
                        f"Avito channel not found for cashbox {cashbox_id}. Channel should be created via /connect endpoint first."
                    )
                    return None
            else:
            ...
            if channel_type == "AVITO" and channel:
                try:
                    ...
                    client = await create_avito_client(
                        ...
                        cashbox_id=cashbox_id,
                        on_token_refresh=lambda token_data: save_token_callback(
                            channel["id"], cashbox_id, token_data
                        ),
                    )
                    if client:
    ...
    def _extract_message_content(
        ...
        if message_type == "text":
            ...
            if isinstance(content, dict) and "voice" in content:
                ...
                voice_url = content["voice"].get("url") or content["voice"].get(
                    "voice_url"
                )
