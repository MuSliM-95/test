...
from database.db import channels, database
...
async def update_avito_channels_icons():
    ...
    logger.info(
        f"Found {len(avito_channels)} Avito channels without icons. Updating..."
    )
    ...
    for channel in avito_channels:
        try:
            ...
            updated_count += 1
            logger.info(
                f"Updated icon for Avito channel {channel['id']} ({channel['name']})"
            )
