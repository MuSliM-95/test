...
async def connect_avito_channel(
    ...
    try:
        ...
        if not access_token:
            raise HTTPException(
                status_code=500, detail="Failed to obtain access token from Avito API"
            )
        ...
        if not avito_channel:
            channel_name = (
                f"Avito - {avito_account_name}"
                if avito_account_name
                else f"Avito - Cashbox {cashbox_id}"
            )
            ...
            if existing_channel:
                channel_name = (
                    f"Avito - {avito_account_name} ({cashbox_id})"
                    if avito_account_name
                    else f"Avito - Cashbox {cashbox_id}"
                )
...
async def get_avito_chats(
    ...
    try:
        ...
        if not avito_channels:
            raise HTTPException(
                status_code=404, detail="Avito channel not configured for this cashbox"
            )
        ...
        for avito_channel in avito_channels:
            try:
                client = await create_avito_client(
                    channel_id=avito_channel["id"],
                    cashbox_id=cashbox_id,
                    on_token_refresh=lambda token_data, ch_id=avito_channel[
                        "id"
                    ]: save_token_callback(ch_id, cashbox_id, token_data),
                )
                        ...
                        if existing_chat:
                            ...
                            if existing_chat.get("chat_contact_id"):
                                ...
                                if client_user_id:
                                    ...
                                    if existing_contact and not existing_contact.get(
                                        "external_contact_id"
                                    ):
        ...
        for chat in all_avito_chats:
            ...
            if last_message and isinstance(last_message, dict):
                last_message_clean = {
                    k: v for k, v in last_message.items() if k != "chat_id"
                }
                last_message_clean = {
                    k: v for k, v in last_message_clean.items() if k != "author_id"
                }
            else:
...
async def get_avito_chat_messages(
    chat_id: str,
    limit: int = Query(
        50, ge=1, le=100, description="Количество сообщений для получения"
    ),
    offset: int = Query(0, ge=0, description="Смещение для пагинации"),
    user=Depends(get_current_user),
):
    try:
        ...
        avito_messages = await client.get_messages(
            chat_id=chat["external_chat_id"], limit=limit, offset=offset
        )
        ...
        for avito_msg in avito_messages:
            ...
            if msg_text and (
                msg_type == "system"
                or "[Системное сообщение]" in msg_text
                or "системное" in msg_text.lower()
            ):
        ...
        if extracted_phone and chat.get("phone") != extracted_phone:
            try:
                ...
                await database.execute(
                    ...
                    .values(phone=extracted_phone, updated_at=datetime.utcnow())
                )
            ...
            try:
                ...
                if isinstance(content, dict):
                    if message_type_str == "text":
                        ...
                        message_text = link_data.get(
                            "text", link_data.get("url", "[Ссылка]")
                        )
                        ...
                        message_text = (
                            f"Объявление: {item_data.get('title', '[Объявление]')}"
                        )
                    elif message_type_str == "location":
                        loc_data = content.get("location", {})
                        message_text = loc_data.get(
                            "text", loc_data.get("title", "[Геолокация]")
                        )
                    elif message_type_str == "voice":
                        voice_data = content.get("voice", {})
                        if isinstance(voice_data, dict):
                            duration = voice_data.get("duration")
                            voice_url = voice_data.get("url") or voice_data.get(
                                "voice_url"
                            )
                            voice_id = voice_data.get("voice_id")
                            if not voice_url and voice_id:
                                try:
                                    voice_url = await client.get_voice_file_url(
                                        voice_id
                                    )
                                except Exception as e:
                                    logger.warning(
                            ...
                            if voice_url:
                                if duration and isinstance(duration, (int, float)):
                                    message_text = (
                                        f"[Voice message: {duration}s - {voice_url}]"
                                    )
                                else:
                                ...
                                if duration and isinstance(duration, (int, float)):
                                    ...
                                    message_text = (
                                        f"[Voice message: voice_id: {voice_id}]"
                                    )
                            else:
                                if duration and isinstance(duration, (int, float)):
                ...
                is_read = (
                    avito_msg.get("is_read", False) or avito_msg.get("read") is not None
                )
                ...
                if message_type_str in ["image", "voice"] and isinstance(content, dict):
                    try:
                        ...
                        if message_type_str == "image" and "image" in content:
                            ...
                            if isinstance(voice_data, dict):
                                file_url = voice_data.get("url") or voice_data.get(
                                    "voice_url"
                                )
                                if not file_url:
                                    voice_id = voice_data.get("voice_id")
...
@router.get(
    "/chats/{chat_id}/applicant/phone", response_model=AvitoApplicantPhoneResponse
)
async def get_avito_applicant_phone(chat_id: str, user=Depends(get_current_user)):
    ...
    try:
            ...
            if client:
                try:
                        ...
                        for user in users:
                            ...
                            if user_id_in_chat and user_id_in_chat != avito_user_id:
                                ...
                                if not applicant_name:
                                    applicant_name = user.get("name") or user.get(
                                        "profile_name"
                                    )
                    ...
                    if not applicant_phone:
                        try:
                            ...
                            for msg in messages:
                                ...
                                if msg_text and (
                                    msg_type == "system"
                                    or msg_text.startswith("[Системное сообщение]")
                                ):
                                    extracted_phone = extract_phone_from_text(msg_text)
                                    if extracted_phone:
                                        applicant_phone = extracted_phone
                                        logger.info(
                                            f"Found phone in system message: {applicant_phone}"
                                        )
                                        break
                                ...
                                for msg in messages:
                                    ...
                                    if msg_text and msg_type == "text":
                                        ...
                                        if extracted_phone:
                                            applicant_phone = extracted_phone
                                            logger.info(
                                                f"Found phone in text message: {applicant_phone}"
                                            )
                                            break
                        except Exception as e:
                            logger.warning(
                                f"Could not extract phone from messages: {e}"
                            )
                    ...
                    if (
                        ...
                        logger.info(
                            f"Chat {chat['external_chat_id']} requires subscription (402) for getting applicant phone"
                        )
                    else:
                        logger.warning(
                            f"Could not get applicant phone from Avito API: {e}"
                        )
        ...
        if applicant_phone and chat.get("chat_contact_id"):
            ...
            await database.execute(
                chat_contacts.update()
                .where(chat_contacts.c.id == chat["chat_contact_id"])
                .values(**update_values)
            )
        ...
        return AvitoApplicantPhoneResponse(
            success=True, chat_id=chat["id"], phone=applicant_phone, name=applicant_name
        )
        ...
        raise HTTPException(
            status_code=400, detail=f"Error getting applicant phone: {str(e)}"
        )


@router.get("/chats/{chat_id}/metadata", response_model=AvitoChatMetadataResponse)
async def get_avito_chat_metadata(chat_id: str, user=Depends(get_current_user)):
    ...
    try:
        ...
        if client:
            try:
                ...
                if (
                    ...
                    logger.info(
                        f"Chat {chat['external_chat_id']} requires subscription (402) for getting metadata from Avito API"
                    )
                else:
        ...
        raise HTTPException(
            status_code=400, detail=f"Error getting chat metadata: {str(e)}"
        )


@router.post("/sync", response_model=AvitoSyncResponse)
async def sync_avito_messages(user=Depends(get_current_user)):
    try:
        cashbox_id = user.cashbox_id
        ...
        for avito_channel in avito_channels:
            try:
                ...
                if not client:
                    logger.warning(
                        f"Could not create Avito API client for channel {avito_channel['id']}"
                    )
                    errors.append(
                        f"Could not create client for channel {avito_channel['id']}"
                    )
                    continue
                    ...
                    try:
                        ...
                        try:
                            ...
                            if (
                                ...
                                logger.info(
                                    f"Chat {chat['id']} (external: {external_chat_id}) requires subscription (402). Skipping."
                                )
                                continue
                            raise
                            ...
                            try:
                                ...
                                if isinstance(content, dict):
                                    ...
                                    message_text = (
                                        str(content)
                                        if content
                                        else f"[{message_type_str}]"
                                    )

                                direction = avito_msg.get("direction", "in")
                                sender_type = (
                                    "CLIENT" if direction == "in" else "OPERATOR"
                                )
                ...
                errors.append(
                    f"Failed to process channel {avito_channel['id']}: {str(e)}"
                )
                continue
...
async def load_avito_history(
    ...
    try:
        ...
        logger.info(
            f"Loading Avito history for channel {channel_id} from timestamp {from_date}"
        )
        ...
        while offset < max_offset:
            try:
                ...
                filtered_chats = [
                    chat for chat in avito_chats if chat.get("created", 0) >= from_date
                ]

                all_chats.extend(filtered_chats)

                oldest_chat_time = min(
                    (chat.get("created", 0) for chat in avito_chats), default=0
                )
                if oldest_chat_time < from_date:
                    logger.info(
                        f"Reached chats older than from_date at offset {offset}. Stopping pagination."
                    )
                    break
                ...
                if "400" in error_str or offset >= 500:
                    logger.warning(
                        f"Stopping pagination at offset {offset} due to error or large offset: {e}"
                    )
                    break
                if (
                    "402" in error_str
                    ...
                    logger.info(
                        f"Stopping pagination at offset {offset} due to subscription required (402)"
                    )
                    break
        ...
        for avito_chat in all_chats:
            try:
                ...
                try:
                    avito_messages = await client.sync_messages(
                        chat_id=external_chat_id, since_timestamp=from_date
                    )
                    ...
                    if (
                        ...
                        logger.info(
                            f"Chat {external_chat_id} requires subscription (402). Skipping messages."
                        )
                    else:
                        logger.warning(
                ...
                if (
                    ...
                    logger.info(
                        f"Chat {avito_chat.get('id')} requires subscription (402). Skipping."
                    )
                else:
                    logger.warning(
...
@router.get("/status")
async def get_avito_status(user=Depends(get_current_user)):
    try:
        cashbox_id = user.cashbox_id
...
async def register_avito_webhook(
    request: AvitoWebhookRegisterRequest, user=Depends(get_current_user)
):
    try:
        cashbox_id = user.cashbox_id
...
async def update_all_avito_webhooks(user=Depends(get_current_user)):
    try:
        cashbox_id = user.cashbox_id
        ...
        for avito_channel in avito_channels:
            ...
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"Webhook updated for channel {channel_id} ({channel_name})"
                    )
                except Exception as webhook_error:
                    failed_count += 1
                    results.append(
                    ...
                    logger.warning(
                        f"Failed to update webhook for channel {channel_id}: {webhook_error}"
                    )
...
async def avito_oauth_authorize(user=Depends(get_current_user)):
    try:
        cashbox_id = user.cashbox_id
        ...
        encoded_params = urllib.parse.urlencode(
            oauth_params, safe=":,", quote_via=urllib.parse.quote
        )
...
async def avito_oauth_callback_hook(
    ...
    try:
        ...
        traceback.print_exc()
        raise HTTPException(
            status_code=500, detail=f"Internal error during OAuth callback: {str(e)}"
        )


@router.get("/oauth/debug")
async def avito_oauth_debug(request: Request):
    return {
        "url": str(request.url),
