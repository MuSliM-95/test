...
async def add_to_favorites(favorite_request: schemas.FavoriteRequest):
    ...
    try:
        ...
        # Разрешаем только товары. Поддержку локаций отключили.
        if favorite_request.entity_type not in ["product"]:
        ...
        #     # Проверяем, что локация существует и публичная — ОТКЛЮЧЕНО
