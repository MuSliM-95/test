...
class TgBillsService:
    ...
    def validate_inn(self, inn: str) -> bool:
        ...
        try:
            if len(inn) == 10:
                ...
                check11 = (
                    sum(int(c) * w for c, w in zip(inn[:10], weights_12)) % 11 % 10
                )
                weights_12[-1] = 8
                check12 = (
                    sum(int(c) * w for c, w in zip(inn[:11], weights_12)) % 11 % 10
                )
    ...
    def find_counterparty_account_number(
        self, text: str, control_number: str
    ) -> tuple[str, str]:
    ...
    def process_text_test(self, text: str) -> dict:
        ...
        result = {}
        result["counterparty_bank_bic"] = (
            re.search(patterns["bic"], text).group(1)
            if re.search(patterns["bic"], text)
            else None
        )
        if result["counterparty_bank_bic"]:
            text = text.replace(result["counterparty_bank_bic"], "")
        counterparty_account_number, text = self.find_counterparty_account_number(
            text, result.get("counterparty_bank_bic", "")[-3:]
        )
    ...
    def process_text_rus(self, text: str) -> dict:
        ...
        result["payment_amount"] = (
            re.search(patterns["sum"], text).group(1)
            if re.search(patterns["sum"], text)
            else None
        )
        if result["payment_amount"]:
            text = text.replace(result["payment_amount"], "")
            result["payment_amount"] = (
                result["payment_amount"].replace(" ", "").replace(",", ".")
            )
            result["payment_amount"] = float(result["payment_amount"])
        result["payment_purpose"] = (
            re.search(patterns["reason"], text).group(1)
            if re.search(patterns["reason"], text)
            else None
        )
        ...
        if result["payment_purpose"]:
            ...
            result["payment_purpose"] = (
                re.search(patterns["reason_2"], text).group(1)
                if re.search(patterns["reason_2"], text)
                else None
            )
            if result["payment_purpose"]:
                text = text.replace(result["payment_purpose"], "")
