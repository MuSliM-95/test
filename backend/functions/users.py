...
async def raschet(user, token):
    update = await database.fetch_all(
        "SELECT * FROM rachet(:token, :today)",
    ...
    z_pbs = [
        dict(pb)
        for pb in payboxes_ids
        if pb.id not in [pb.pb_id for pb in update if pb.pb_id]
    ]
    z_proj = [
        dict(pr)
        for pr in projects_ids
        if pr.id not in [pr.pr_id for pr in update if pr.pr_id]
    ]
    ...
    for z_project in z_proj:
        q = (
            projects.update()
            .where(projects.c.id == z_project["id"])
            .values(
                {
                    "incoming": 0,
                    "outgoing": 0,
