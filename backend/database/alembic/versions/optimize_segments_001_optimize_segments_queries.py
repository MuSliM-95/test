...
def upgrade():
    ...
    ВАЖНО: Эта миграция выполняется БЕЗ транзакции, так как CREATE INDEX CONCURRENTLY
    не может выполняться внутри транзакционного блока.

    Создание индексов займет 1-5 минут в зависимости от размера данных.
    """

    # Получаем connection и отключаем транзакцию для CONCURRENTLY индексов
    connection = op.get_bind()

    # ==================================================================================
    # Шаг 1: Включение расширения pg_trgm (выполняется в транзакции - это нормально)
    # ==================================================================================

    connection.execute("COMMIT")  # Завершаем текущую транзакцию
    connection.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")

    # ==================================================================================
    # КРИТИЧЕСКИЕ индексы (выполняются БЕЗ транзакции)
    # ==================================================================================
    ...
    connection.execute("ANALYZE loyality_cards")
    connection.execute("ANALYZE loyality_transactions")
    connection.execute("ANALYZE docs_sales")
    connection.execute("ANALYZE docs_sales_goods")
    connection.execute("ANALYZE nomenclature")
    connection.execute("ANALYZE categories")
    connection.execute("ANALYZE contragents")
    connection.execute("ANALYZE contragents_tags")
    connection.execute("ANALYZE docs_sales_tags")
    connection.execute("ANALYZE docs_sales_delivery_info")
...
def downgrade():
    ...
    DROP INDEX CONCURRENTLY также требует выполнения БЕЗ транзакции.
    """

    # Получаем connection и отключаем транзакцию
    connection = op.get_bind()
    connection.execute("COMMIT")  # Завершаем текущую транзакцию

    # Удаляем индексы в обратном порядке (БЕЗ точек с запятой!)
    connection.execute(
    ...
    connection.execute(
        "DROP INDEX CONCURRENTLY IF EXISTS idx_docs_sales_tags_docs_name"
    )
    connection.execute(
        "DROP INDEX CONCURRENTLY IF EXISTS idx_docs_sales_contragent_aggregate"
    )
    connection.execute(
        "DROP INDEX CONCURRENTLY IF EXISTS idx_contragents_id_phone_cashbox"
    )
    connection.execute("DROP INDEX CONCURRENTLY IF EXISTS idx_categories_name_trgm")
    connection.execute("DROP INDEX CONCURRENTLY IF EXISTS idx_nomenclature_category")
    connection.execute(
